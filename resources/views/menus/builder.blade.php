...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i>{{trans('voyager.Menu Builder')}} ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="voyager-plus"></i> {{trans('voyager.New Menu Item')}}</div>
    </h1>
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">@lang('voyager.Drag and drop the menu Items below to re-arrange them').</p>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 id="m_hd_add" class="modal-title hidden"><i class="voyager-plus"></i> @lang('voyager.Create a New Menu Item')</h4>
                    <h4 id="m_hd_edit" class="modal-title hidden"><i class="voyager-edit"></i> @lang('voyager.Edit Menu Item')</h4>
                </div>
                <form action="" id="m_form" method="POST"
                    ...
                    <div class="modal-body">
                        @include('voyager::multilingual.language-selector')
                        <label for="name">@lang('voyager.Title of the Menu Item')</label>
                        @include('voyager::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                        <input type="text" class="form-control" id="m_title" name="title" placeholder="@lang('voyager.Title')"><br>
                        <label for="type">@lang('voyager.Link type')</label>
                        <select id="m_link_type" class="form-control" name="type">
                            <option value="url" selected="selected">@lang('voyager.Static URL')</option>
                            <option value="route">@lang('voyager.Dynamic Route')</option>
                        </select><br>
                        <div id="m_url_type">
                            <label for="url">@lang('voyager.URL for the Menu Item')</label>
                            <input type="text" class="form-control" id="m_url" name="url" placeholder="URL"><br>
                        </div>
                        <div id="m_route_type">
                            <label for="route">@lang('voyager.Route for the menu item')</label>
                            <input type="text" class="form-control" id="m_route" name="route" placeholder="@lang('voyager.Route')"><br>
                            <label for="parameters">@lang('voyager.Route parameters (if any)')</label>
                        ...
                        <label for="icon_class">{{trans("voyager.Font Icon class for the Menu Item (Use a")}} <a
                                    href="{{ voyager_asset('fonts/voyager/icons-reference.html') }}"
                                    target="_blank">{{trans('voyager.Voyager Font Class')}}</a>）</label>
                        <input type="text" class="form-control" id="m_icon_class" name="icon_class"
                               placeholder="@lang('voyager.Icon Class (optional)')"><br>
                        <label for="color">@lang('voyager.Color in RGB or hex (optional)')</label>
                        <input type="color" class="form-control" id="m_color" name="color"
                               placeholder="Color (ex. #ffffff or rgb(255, 255, 255)"><br>
                        <label for="target">@lang('voyager.Open In')</label>
                        <select id="m_target" class="form-control" name="target">
                            <option value="_self" selected="selected">@lang('voyager.Same Tab/Window')</option>
                            <option value="_blank">@lang('voyager.New Tab/Window')</option>
                        </select>
                        <input type="hidden" name="menu_id" value="{{ $menu->id }}">
                        <input type="hidden" name="id" id="m_id" value="">
                    </div>
                    <div class="modal-footer">
                        <input type="submit" class="btn btn-success pull-right delete-confirm__" value="@lang('voyager.Update')">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">@lang('voyager.Cancel')</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
