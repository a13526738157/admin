        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <table class="table table-striped database-tables">
                    <thead>
                        <tr>
                            <th>@lang('voyager.Table Name')</th>
                            <th>BREAD/CRUD @lang('voyager.Actions')</th>
                            <th style="text-align:right">@lang('voyager.Table Actions')</th>
                        </tr>
                    </thead>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="voyager-trash"></i> @lang('voyager.Are you sure you want to delete the BREAD for the table?') </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.database.bread.delete', ['id' => null]) }}" id="delete_builder_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="submit" class="btn btn-danger" value="@lang('voyager.Yes, remove the BREAD')">
                    </form>
                    <button type="button" class="btn btn-outline pull-right" data-dismiss="modal">@lang('voyager.Cancel')</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-trash"></i> @lang('voyager.Are you sure you want to delete the') <span
                                id="delete_table_name"></span> @lang('voyager.table')?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.database.destroy', ['database' => '__database']) }}" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="submit" class="btn btn-danger pull-right" value="@lang('voyager.Yes, delete this table')">
                        <button type="button" class="btn btn-outline pull-right" style="margin-right:10px;"
                                data-dismiss="modal">@lang('voyager.Cancel')
                        </button>
                    </form>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body" style="overflow:scroll">
                    <table class="table table-striped">
                        <thead>
                        <tr>
                            <th>@lang('voyager.Field')</th>
                            <th>@lang('voyager.Type')</th>
                            <th>@lang('voyager.Null')</th>
                            <th>@lang('voyager.Key')</th>
                            <th>@lang('voyager.Default')</th>
                            <th>@lang('voyager.Extra')</th>
                        </tr>
                        </thead>
                        <tbody>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline pull-right" data-dismiss="modal">@lang('voyager.Close')</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
        ...
        $(function () {
            ...
            $('td.actions').on('click', '.delete_table', function (e) {
                ...
                if ($(this).hasClass('remove-bread-warning')) {
                    toastr.warning("@lang('voyager.Please make sure to remove the BREAD on this table before deleting the table.')");
                } else {
