...
@section('page_header')
    <div class="page-title">
        <i class="voyager-data"></i> @if(isset($dataType->id)){{ trans('voyager.Edit BREAD for',['table'=>ucfirst($dataType->name)])}}@elseif(isset($table)){{ 'Create BREAD for ' . $table . ' table' }}@endif
    </div>
    @php
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ ucfirst($table) }} Bread @lang('voyager.info')</h3>
                        </div>

                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="name">@lang('voyager.Table Name')</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('voyager.Display Name (Singular)')</label>
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="@lang('voyager.Display Name (Singular)')"
                                           value="@if(isset($dataType->display_name_singular)){{ $dataType->display_name_singular }}@else{{ $display_name }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('voyager.Display Name (Plural)')</label>
                                    @if($isModelTranslatable)
                                        @include('voyager::multilingual.input-hidden', [
                                            'isModelTranslatable' => true,
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="@lang('voyager.Display Name (Plural)')"
                                           value="@if(isset($dataType->display_name_plural)){{ $dataType->display_name_plural }}@else{{ $display_name_plural }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('voyager.URL Slug (must be unique)')</label>
                                    <input type="text" class="form-control" name="slug" placeholder="ex. /admin/posts"
                                           value="@if(isset($dataType->slug)){{ $dataType->slug }}@else{{ $slug }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('voyager.Icon (optional) Use a') <a
                                                href="{{ voyager_asset('fonts/voyager/icons-reference.html') }}"
                                                target="_blank">@lang('voyager.Voyager Font Class')</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="@lang('voyager.Voyager Font Class')"
                                           value="@if(isset($dataType->icon)){{ $dataType->icon }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('voyager.Model Name')</label>
                                    <span class="glyphicon glyphicon-question-sign"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="ex. \App\User, @lang('voyager.if left empty will try and use the table name')"></span>
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('voyager.Controller Name')</label>
                                    <span class="glyphicon glyphicon-question-sign"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="ex. PageController, @lang('voyager.if left empty will use the BREAD Controller')"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="@lang('voyager.Controller Name')"
                                           value="@if(isset($dataType->controller)){{ $dataType->controller }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="generate_permissions">@lang('voyager.Generate Permissions')</label><br>
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="server_side">@lang('voyager.Server-side Pagination')</label><br>
                            ...
                            <div class="form-group">
                                <label for="description">@lang('voyager.Description')</label>
                                <textarea class="form-control" name="description"
                                          placeholder="@lang('voyager.Description')"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title">{{trans('voyager.Edit the rows',['table'=>ucfirst($table)])}}</h3>
                        </div>

                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">@lang('voyager.Field')</div>
                                <div class="col-xs-2">@lang('voyager.Visibility')</div>
                                <div class="col-xs-2">@lang('voyager.Input Type')</div>
                                <div class="col-xs-2">@lang('voyager.Display Name')</div>
                                <div class="col-xs-4">@lang('voyager.Optional Details')</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        ...
                                        <strong>@lang('voyager.Type'):</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>@lang('voyager.IndexKey'):</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>@lang('voyager.Required'):</strong>
                                        @if($data['null'] == "NO")
                                            <span>@lang('voyager.Yes')</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}"
                                                   checked="checked">
                                        @else
                                            <span>@lang('voyager.No')</span>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                    {{ 'checked="checked"' }}
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">@lang('voyager.Browse')</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">@lang('voyager.Read')</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">@lang('voyager.Edit')</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">@lang('voyager.Add')</label><br/>
                                        <input type="checkbox"
                                               ...
                                               name="field_delete_{{ $data['field'] }}" @if(isset($dataRow->delete) && $dataRow->delete){{ 'checked="checked"' }}@elseif($data['key'] == 'PRI')@elseif($data['type'] == 'timestamp' && $data['field'] == 'updated_at')@elseif(!isset($dataRow->delete)){{ 'checked="checked"' }}@endif>
                                                <label for="field_delete_{{ $data['field'] }}">@lang('voyager.Delete')</label><br/>
                                    </div>
                                    <div class="col-xs-2">
                            ...
                            <div class="box-footer">
                                <button type="submit" class="btn btn-primary">@lang('voyager.Submit')</button>
                            </div>
                        </div><!-- .panel-body -->
                    </div><!-- .panel -->
    ...
    <script>
        ...
        $(function () {
            ...
            $('textarea[data-editor]').each(function () {
                ...
                editor.setTheme("ace/theme/github");//主题
                _session.setMode("ace/mode/json");//程序语言
                if (textarea.val()) {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('{{trans('voyager.Seems like you introduced some invalid JSON.')}}', '{{trans('voyager.Validation errors')}}', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
