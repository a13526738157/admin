                    ...
                    @foreach($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @elseif($row->type == 'multiple_images')
                                @foreach(json_decode($dataTypeContent->{$row->field}) as $v)
                                    <img class="img-responsive"
                                         src="{{ Voyager::image($v) }}" style="display: inline-block;width: 200px;height: 200px">
                                @endforeach
                            @elseif($row->type == 'select_dropdown' && isset($rowDetails->relationship->options) &&
                                    !empty($rowDetails->options->{$dataTypeContent->{$row->field}})
                            )

                                <?php echo $rowDetails->relationship->options->{$dataTypeContent->{$row->field}};?>
                            ...
                            @elseif($row->type == 'checkbox')
                                ...
                                @endif
                            @elseif($row->type == 'radio_btn' && isset($rowDetails->relationship->options) &&!empty($rowDetails->options->{$dataTypeContent->{$row->field}}))
                                <?php echo $rowDetails->relationship->options->{$dataTypeContent->{$row->field}};?>
