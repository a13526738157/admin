...
abstract class Controller extends BaseController
{
    ...
    public function validateBread($request, $data)
    {
        ...
        foreach ($data as $row) {
            ...
            switch ($fieldType){
                case 'email':
                    $rules[$row->field] = ['email'];
                    break;
                case 'link':
                    $rules[$row->field] = ['active_url'];
                    break;
            }
            if (isset($options->validation)) {
                if (isset($options->validation->rule)) {
                    if (!is_array($options->validation->rule)) {
                        if(isset($rules[$row->field]))
                             $rules[$row->field] = array_merge($rules[$row->field],explode('|', $options->validation->rule));
                        else
                            $rules[$row->field] = explode('|', $options->validation->rule);
                    } else {
                        if(isset($rules[$row->field]))
                            $rules[$row->field] = array_merge($rules[$row->field],$options->validation->rule);
                        else
