...
class VoyagerDatabaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()
               ...
               ->with($this->alertSuccess(trans("voyager.Successfully created")));
        } catch (Exception $e) {
    ...
    public function edit($table)
    {
        ...
        if (!SchemaManager::tableExists($table)) {
            return redirect()
                ...
                ->with($this->alertError(trans("voyager.The table you want to edit doesn't exist")));
        }
    ...
    public function update(Request $request)
    {
        ...
        return redirect()
               ->route('voyager.database.edit', $table['name'])
               ->with($this->alertSuccess(trans("Successfully updated")));
    }
    ...
    public function destroy($table)
    {
        ...
        try {
            ...
            return redirect()
                ->route('voyager.database.index')
                ->with($this->alertSuccess(trans("voyager.Successfully deleted")));
        } catch (Exception $e) {
    ...
    public function storeBread(Request $request)
    {
        ...
        try {
            ...
            $data = $dataType->updateDataType($request->all(), true)
                ? $this->alertSuccess(trans('Successfully created'))
                : $this->alertError(trans('voyager.Sorry it appears there may have been a problem creating'));

            return redirect()->route('voyager.database.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('voyager.database.index')->with($this->alertException($e, trans('voyager.Saving').trans('voyager.Failed')));
        }
    }
    ...
    public function updateBread(Request $request, $id)
    {
        ...
        try {
            ...
            $data = $dataType->updateDataType($request->all(), true)
                ? $this->alertSuccess(trans("voyager.Successfully updated"))
                : $this->alertError(trans('voyager.Sorry it appears there may have been a problem updating'));
            ...
            return back()->with($this->alertException($e, trans('voyager.Saving').trans('voyager.Failed')));
        }
    }
    ...
    public function deleteBread($id)
    {
        ...
        $data = Voyager::model('DataType')->destroy($id)
            ? $this->alertSuccess(trans("voyager.Successfully removed deleted"))
            : $this->alertError(trans('voyager.Sorry it appears there may have been a problem deleting'));
